...
public class CustomerServ extends SeleniumFunc implements CustomerServLoc {
	...
	//code to select Add Task button
			...
			/*	WebElement AddTaskBtn=driver.findElement(By.xpath(addtaskbtn_xpath));
				...
				AddTaskBtn.click();*/
		/*
		 * driver.switchTo().defaultContent(); // PegaGadget2Ifr
		 * 
		 * waitFor(1); driver.switchTo().frame("PegaGadget2Ifr"); waitFor(1); WebElement
		 * ele = driver.findElement(By.xpath(addtaskbtn_xpath)); waitForElement(ele, 1);
		 * Actions action = new Actions(driver);
		 * action.moveToElement(ele).click().build().perform(); waitFor(1);
		 */
		...
		//code to select Wrap Up button
				public void clickWrapUpButton() throws Exception  {
					SeleniumFunc.xpath_GenericMethod_Click(wrapupbtn_xpath);
		/*
		 ...
		 */
			...
			public void selectServiceCase(String casetext)  {
			//	waitFor(2);
			//	driver.findElement(By.xpath("//a[@title='"+casetext+"']")).click();
				try {
					SeleniumFunc.xpath_GenericMethod_Click("//a[text()='"+casetext+"']");
				} catch (Exception e) {
	 ...
	 */
			...
			public void clickOnNewButton() throws Exception {
					...
					Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(new_link_xpath));
					SeleniumFunc.xpath_GenericMethod_Click(new_link_xpath);
				
			}
			
			public void clickOnPhoneCallInteraction() throws Exception {
				Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(phonecall_interaction_xpath));
				SeleniumFunc.xpath_GenericMethod_Click(phonecall_interaction_xpath);
				
			}
			
			public void clickOnSearchButton() throws Exception {
				SeleniumFunc.xpath_Genericmethod_verifyElementPresent(xpath_search_button);
				SeleniumFunc.xpath_GenericMethod_Click(xpath_search_button);
				
			}
			
			public void clickOnSearchResultRow() throws Exception {
			
					waitFor(3);
					Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(search_result_xpath));
					SeleniumFunc.xpath_GenericMethod_Click(search_result_xpath);
				
			}
			
			public void clickOnPersonalQuestionOne() throws Exception {
				
			Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(xpath_personalQuestionOne));
			SeleniumFunc.xpath_GenericMethod_Click(xpath_personalQuestionOne);
				
			}
			
			public void clickOnPersonalQuestionTwo() throws Exception {
				Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(xpath_personalQuestionTwo));
				SeleniumFunc.xpath_GenericMethod_Click(xpath_personalQuestionTwo);
				
			}
			
			public void clickOnContactQuestionOne() throws Exception {
				Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(xpath_contactQuestionOne));
					SeleniumFunc.xpath_GenericMethod_Click(xpath_contactQuestionOne);
				
			}
			
			public void clickOnSubmitWrapUp() throws Exception {
				Assert.assertTrue(SeleniumFunc.xpath_Genericmethod_verifyElementPresent(xpath_submitbtn_wrapup));
				SeleniumFunc.xpath_GenericMethod_Click(xpath_submitbtn_wrapup);
			}
}
